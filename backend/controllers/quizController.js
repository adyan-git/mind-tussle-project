...
export const createQuiz = async (req, res) => {
    ...
    try {
        const { role, id: userId } = req.user;
        const { title, category, passingMarks, questions = [] } = req.body;
        ...
        const normalizedQuestions = Array.isArray(questions)
            ? questions
                .map((question) => {
                    ...
                    const rawAnswer = String(question?.correctAnswer || "").trim().toUpperCase();
                    const normalizedAnswer = ["A", "B", "C", "D"].includes(rawAnswer) ? rawAnswer : "A";
                    return {
        ...
        const resolvedPassingMarks = Number.isFinite(Number(passingMarks))
            ? Math.max(0, Math.min(totalMarks, Number(passingMarks)))
            : Math.ceil(totalMarks / 2);

        const newQuiz = new Quiz({
            title,
            category,
            duration: totalMarks * 2,
            totalMarks,
